...
- 会话管理支持用户、日期、模型、数字员工和归档状态筛选，显示风险、命中词、耗时与 Token，并支持归档、批量删除和 PDF 导出
- 系统设置执行类型/范围校验，注册、维护模式、默认模型、大屏刷新、采集超时/数量、上传大小和会话时长由运行时代码读取
- 管理端新增/编辑/删除/启停操作统一写入可筛选审计日志；外部源代码扫描按团队安排交由 MonkeyScan，不属于系统内操作审计
...
- 采集任务统一为等待、运行、成功、部分成功、失败和已取消，支持真实进度、步骤日志、取消、失败重试及启动恢复
- 来源列表从采集任务聚合最近时间、成功/失败次数、成功率、耗时与最近错误，并提供手动执行入口
...
- 支持来源/日期/风险筛选、单条重采、批量删除/深采、CSV 导出和显式去重；重复入仓与舆情预警均保持幂等
- 稳定问数接口覆盖采集量、来源数量、七日趋势、失败率、高风险、关键词频率和平均耗时，返回结论、KPI、图表与表格
...
- 接口型员工运行时读取后台绑定接口的 URL、方法、参数、响应路径、超时与重试，并保留 SSRF、响应大小和敏感请求头限制
- 已启用 Skills 在每次执行时动态加载；提示词 Skill 改变最终提示词，数据库问数 Skill 调用真实 `QueryIntentService`，停用后立即失效
- TTS 使用数据库配置、长文本分段、音频缓存和调用日志；生图使用异步 OpenAI 兼容服务与数据库任务，未配置生视频时明确失败且不返回假地址
...
完整回归当前为 97 项，覆盖权限与会话隔离、采集任务生命周期、来源指标、仓库操作、七类问数、接口与 Skills、TTS、多模态任务、统一舆情、设置生效、操作审计，以及成员 D 的 PDF、人脸、手势、语音和双大屏能力。验收同时执行 Ruff、Python 编译、全部前端 JavaScript 语法检查、空库/旧库迁移与 HTTP 冒烟。
